    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(this->ui->action_obre_dades_multiples_shaders, SIGNAL(triggered()), glWidget, SLOT(activaDadesMultipleShading()));


    connect(this->ui->action_bgrefraction, SIGNAL(triggered()), glWidget, SLOT(activaBGTransparency()));
    connect(this->ui->action_bgreflection, SIGNAL(triggered()), glWidget, SLOT(activaBGReflection()));
    ...
    connect(builder, SIGNAL(newData(Scene*)),glWidget, SLOT(newData(Scene *)));
    connect(builder, SIGNAL(demo1(Scene*)), glWidget, SLOT(startDemo1(Scene*)));
    connect(builder, SIGNAL(demo2(Scene*)), glWidget, SLOT(startDemo2(Scene*)));
    ...
    connect(dialeg, SIGNAL(acceptatOK(QString, QString, QString, QString, double, double, double, double)), builder, SLOT(newDataScene(QString, QString, QString, QString, double, double, double, double)));
    connect(dialeg, SIGNAL(cancelOK()), this, SLOT(closeDialogDades()));

    connect(this->ui->actionSpace_Scene, SIGNAL(triggered()), builder, SLOT(newSceneDemo1()));
    connect(this->ui->actionMountain_Scene, SIGNAL(triggered()), builder, SLOT(newSceneDemo2()));
...
void MainWindow::closeDialogDades() {
    // TODO Fase 1 mostrar la finestra de dialeg del visualization mapping del fitxer de dades:
    // En ella es demana el fitxer de dades geolocalitzades, les dimensions del mon virtual, l'escala minima i màxima, el colormap a usar,
    // i la textura a posar.
    dialeg->hide();
...
void MainWindow::on_textureNormalFileEdit_returnPressed()
{
    glWidget->setNormalTextureFile(ui->textureNormalFileEdit->text());
}

void MainWindow::on_textureBackgroundFileEdit_returnPressed()
{
    glWidget->setBackgroundTextureFile(ui->textureBackgroundFileEdit->text());
}
...
void MainWindow::on_browseTextureBtn_Normal_pressed()
{
    QString file = QFileDialog::getOpenFileName(this,
                                                "Open Texture",
                                                QStandardPaths::standardLocations(QStandardPaths::DesktopLocation).at(0),
                                                "*.jpg *.png *.tif *.bmp");
    ui->textureNormalFileEdit->setText(file);
    on_textureNormalFileEdit_returnPressed();
}

void MainWindow::on_browseTextureBtn_Background_pressed()
{
    QString file = QFileDialog::getOpenFileName(this,
                                                "Open Texture",
                                                QStandardPaths::standardLocations(QStandardPaths::DesktopLocation).at(0),
                                                "*.jpg *.png *.tif *.bmp");
    ui->textureBackgroundFileEdit->setText(file);
    on_textureBackgroundFileEdit_returnPressed();
}
